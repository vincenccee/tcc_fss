...
Os \textit{Hyperplane-defined Functions} (HDFs) foram projetados para representar a forma como as pesquisas do GA ocorrem, através da combinação da construção de blocos \cite{holland2000building}. As HDFs são definidas sobre o conjunto de todas as palavras de um determinado comprimento $n$. O \textit{fitness} de cada palavra é determinado pelo esquemas presente na cadeia de caracteres. Um esquema é uma palavra definida sobre o alfabeto da palavra original com a adição do caractere $*$ e é de comprimento $n$, em que o caractere $*$ corresponderá um 1 ou um 0 em uma cadeia binária (sendo qualquer valor em outras cadeias com um alfabeto maior).

Porém existem duas dificuldades principais na utilização dos HDFs em ambientes dinâmicos. Uma delas é que, mesmo sendo fácil gerar uma nova cadeia aleatória, não é possível geral uma cadeia nova semelhando com uma anterior. A outra dificuldade é que não é fácil determinar qual o ponto ótimo de uma cadeia após sua criação.

Então foram estipuladas três regras que restringem as HDFs e eliminam as dificuldades citadas anteriormente, que são:
...
O \textit{Shaky Ladder Hyperplane-defined Functions} baseia-se no HDF \cite{rand2005shaky}
